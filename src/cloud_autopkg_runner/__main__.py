...
# Shell convention for "terminated by signal N"
...
def _signal_handler(sig: int, _frame: FrameType | None) -> NoReturn:
    ...
    handler logs an error message indicating the signal and exits with
    `128 + signal number`, the shell convention for termination by a signal.
    An interrupted run has not succeeded, so it must not exit 0.
